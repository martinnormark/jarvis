...
This module provides custom audio interfaces that can be interrupted
...
class VolumeReducingAudioInterface(AudioInterface):
    ...
    def _apply_volume_reduction(self, audio: bytes) -> bytes:
        ...
        Args:
            audio: Raw audio data in 16-bit PCM format
        ...
        with self._volume_lock:
            ...
            # Calculate current volume based on fade duration
            elapsed_time = time.time() - self._interrupt_start_time
            fade_duration_sec = self.fade_duration_ms / 1000.0

            if elapsed_time >= fade_duration_sec:
                # Fully faded to reduced volume
                target_volume = self.volume_reduction_factor
            else:
                # Interpolate between full volume and reduced volume
                fade_progress = elapsed_time / fade_duration_sec
                target_volume = 1.0 - (
                    fade_progress * (1.0 - self.volume_reduction_factor)
                )
        ...
        # Convert bytes to 16-bit integers, apply volume, convert back
        audio_data = struct.unpack(f"<{len(audio) // 2}h", audio)
        reduced_audio_data = [int(sample * target_volume) for sample in audio_data]
        return struct.pack(f"<{len(reduced_audio_data)}h", *reduced_audio_data)
